...
//
// Section 8 — resolves the RECEIVING tenant's own WhatsApp token (same
// fallback-to-global-env-vars behavior as src/infra/whatsapp.js's
// credentials(), for the same upgrade-continuity reason) rather than
...
//
// found live: Sarvam defaults to WAV, which WhatsApp's media-upload API
// flatly rejects (only aac/mp4/mpeg/amr/ogg/opus are accepted) — every
// synthesized reply was failing to upload. Requesting mp3 (audio/mpeg)
// explicitly since that's both a codec Sarvam supports and one WhatsApp
// accepts directly, with no transcoding step needed.
async function synthesizeSpeech(text, languageCode) {
  ...
  const resp = await fetch(`${SARVAM_BASE}/text-to-speech`, {
    ...
    // Sarvam's actual required field is `language_code`, not
    // `target_language_code` (found live: sending the wrong field name
    // meant Sarvam never got a language and fell back to defaults for
