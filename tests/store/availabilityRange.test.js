...
let blockSlot, blockedRangesForDay, timeToMinutes, timeSlotsFor, labelToMinutes;
...
before(async () => {
  ...
  ({ blockSlot, blockedRangesForDay, timeToMinutes } = require("../../src/store/availabilityStore"));
  ({ timeSlotsFor, labelToMinutes } = require("../../src/engine/dateSlots"));
});
...
test("2.5: a 2:30-3:40 range block excludes every overlapping slot and nothing else", async () => {
  await blockSlot(TENANT, "t", "p1", FUTURE_DATE, "14:30", "15:40", "lunch");
  const ranges = await blockedRangesForDay(TENANT, "t", "p1", FUTURE_DATE);
...
test("2.1/backward-compat: a legacy single-slot block (no end_time) still excludes exactly that slot", async () => {
  await blockSlot(TENANT, "t", "p2", FUTURE_DATE, "10:00", null, "old-style block");
  const ranges = await blockedRangesForDay(TENANT, "t", "p2", FUTURE_DATE);
...
test("regression: the original format-mismatch bug stays fixed (24h stored time correctly excludes the 12h-labeled slot)", async () => {
  await blockSlot(TENANT, "t", "p3", FUTURE_DATE, "09:30", null, null);
  const ranges = await blockedRangesForDay(TENANT, "t", "p3", FUTURE_DATE);
  const slots = timeSlotsFor(morningWorkflow, FUTURE_DATE, new Set(), ranges);
