...
- **Known, deliberate gap**: `workflows/*.json` remain a single global catalog shared by every tenant — not yet tenant-scoped. Every other piece of Section 8's "fully isolated" guarantee holds; this one doesn't yet, and is called out explicitly in `docs/ARCHITECTURE.md` rather than left as a silent inconsistency. The natural fix (a subdirectory per tenant slug) is a real, separate piece of work, not something to rush alongside everything else here.
...
A real React + Vite rewrite of the dashboard, built at `frontend/` and served at **`/app`** — deliberately alongside the existing hand-rolled `public/dashboard.html` at `/dashboard`, not a replacement for it. Both talk to the exact same `/api/dashboard/*` and `/api/auth/*` routes; no backend changes were needed to add this.

- **Why additive, not a cutover**: this dashboard runs a business with real bookings on it. Rewriting the entire UI surface in one pass and simultaneously flipping the default route it's reachable at would mean the very first time this new code sees production use is also the moment there's no working fallback if something's subtly wrong. `/app` gets a real shakeout first; swapping the default (or removing the old one) is a one-line, easily-reversible follow-up once it has.
- **What's ported, fully working, and live-verified against real data**: login/logout/forgot-password, the provider view (bookings list with filters, Serve/Complete/Cancel/Reschedule/No-show/Refund actions, Availability blocking, Calendar Sync connect/disconnect, Analytics with revenue), the admin view (All Bookings with search/filters, Manage Businesses as a read-only list, Manage Team with add/deactivate, Support Requests, Feedback, Knowledge Base, Audit Log), and Section 11's live SSE updates (a `useLiveEvents` hook wrapping the same `EventSource` stream — the live-status dot and auto-refreshing panels work identically to the classic dashboard).
- **What's deliberately not ported, and why**: the full workflow builder (hand-written step JSON, the AI-drafting modal) and the templates marketplace (publish/install) are real, separate, materially larger pieces of UI than the rest of this rewrite — porting them was a scope decision to defer, not an oversight. Editing a business's own configuration and marketplace templates still work today, unchanged, in the classic dashboard at `/dashboard`; the new `/app` links out to it for that one workflow. `BookingsTable` also renders one generic column set across every workflow (a "Details" column summarizing whichever of reason/age/gender/nights a booking actually has) rather than the classic dashboard's per-workflow column configuration — same information, differently organized, a deliberate simplification rather than a per-industry reimplementation.
...
- **Marketplace** — publish any working business as a reusable template, then install it as a brand-new business (`src/store/templateStore.js`). Installing *copies* the config rather than referencing it, so editing the installed business never mutates the template (and a template edit never silently changes a live business mid-booking-season). Admin only, since installing writes a new `workflows/*.json` that goes live for the bot immediately.
