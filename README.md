...
table (`src/db.js`) with a role — `admin` or `provider` — and a signed
HttpOnly session cookie (`src/auth.js`, HMAC over a JSON payload, no JWT
...
**Migrating from the old `DASHBOARD_ACCESS_KEY` scheme?** That env var is gone. Set `SESSION_SECRET` (generate one — see `.env.example`) and either `ADMIN_BOOTSTRAP_EMAIL`/`ADMIN_BOOTSTRAP_PASSWORD` (fresh install) or insert an admin row directly via `src/userStore.js`'s `users.create()` (existing install, users table not empty). `DASHBOARD_ACCESS_KEY` is no longer read anywhere and can be removed from `.env`.
...
- **Stored-XSS hardened** — customer-typed fields (name, reason) go straight from WhatsApp into the database with no sanitization by design (so WhatsApp confirmations/STATUS show exactly what the customer typed), which means anything that renders them into the dashboard's HTML has to escape them at render time or a message like `<img src=x onerror=...>` as a "name" executes in the admin's browser. Found live and fixed: every dynamic value the dashboard inserts via `innerHTML` now goes through `escapeHtml()` (`public/dashboard.html`) — verified by seeding an actual `<script>`/`<img onerror>` payload and confirming it renders as inert text, not code. Free-text fields are also capped at 200 characters at write time (`src/workflowEngine.js`).
- **Security headers** — `X-Content-Type-Options`, `X-Frame-Options: DENY`, `Referrer-Policy`, and a `Content-Security-Policy` are set on every response (`server.js`). CSP's `script-src` needs `'unsafe-inline'` because the dashboard is one inline `<script>` block rather than an external file — the XSS defense here is the escaping above, not CSP; what CSP still buys is blocking any externally-loaded script/frame/object and clickjacking via `frame-ancestors 'none'`.
...
- **Get a permanent WhatsApp token.** The Meta test token expires every 24 hours — set up a System User token in Meta Business Settings instead. (If `logs/app.log` shows `401 OAuthException` on outbound sends, this is why.)
...
- **`workflows/service.json` (Automobile) is incomplete** — it only has `select_provider` + `select_date` steps, with no name-collection step and no `review_confirm`. A real customer completing it gets a booking with no name captured and no chance to review before it's confirmed. Either finish it (copy the step shape from `medical.json`/`hair.json`) or remove the file before going live, so it doesn't stay reachable as a broken flow.
- **Set up database backups.** `data/bookpilot.db` is the only copy of every booking — there's no automated backup yet. A scheduled copy of the `data/` directory (it's a single SQLite file, safe to copy while the WAL is checkpointed, or just stop the server briefly) is enough at this scale.
...
- **Password reset / email verification / MFA for the dashboard** — login is real (per-user password + signed session + RBAC + audit log), but there's no self-serve "forgot password" flow yet; an admin has to re-create the account via `users.create()`.
