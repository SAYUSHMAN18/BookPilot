...
- **`npm audit`: known vulnerabilities in `vite`/`esbuild`, Vite's dev server only**, accepted rather than silently ignored or forced into an untested major-version jump. Grew from one moderate finding to one moderate (`esbuild`, GHSA-67mh-4wv8-2f99) plus one high (`vite` itself — a path-traversal issue in optimized-deps `.map` handling, an NTLMv2 hash-disclosure issue in `launch-editor` on Windows, and an `fs.deny` bypass on Windows alternate paths) once `vitest`/`@testing-library/*` were added as devDependencies (Item 6 below) and pulled in a slightly different resolution. Every one of these is a `vite dev` issue (a malicious website could probe the dev server, or a crafted path could escape its allowed directories, while a developer has it running and reachable); the actual production deployment serves Vite's static *build output* through Express (`express.static`), which none of these touch at all — and CI's frontend job deliberately has no `npm audit` gate for exactly this reason (see `.github/workflows/ci.yml`'s own comment). Revisit when upgrading to Vite 6+.
...
Self-serve signup (`POST /api/signup`, `public/marketing/signup.html`) creates a tenant + admin and logs them straight into `/app` — that part already existed. The gap it left (at the time): a brand new tenant landed in a full dashboard pre-populated with 5 unrelated demo businesses (haircut, hotel, makeup, medical, automobile) and zero indication of what to actually do next, or whether the demo data was theirs to keep or replace.

> **Superseded** — a later pass (see "Business listing: own real data, not a demo catalog" below) removed the auto-seeded demo catalog entirely. A brand new tenant now starts with **zero** businesses; the checklist item below reflects that.
...
- **`GET /api/dashboard/setup-checklist`** (`SetupChecklistPanel.jsx`, pinned to the top of the admin dashboard) computes four go-live milestones from real, live tenant state — never a separately-tracked "wizard progress" that could drift from what's actually true:
  - *Customize your first business* — `tenantWorkflowStore.hasCustomizations()` (`src/store/tenantWorkflowStore.js`): true once the tenant has any business at all (nothing is auto-seeded any more, so any row is a real signal), or — for the legacy case of a tenant still carrying rows from before auto-seeding was removed — once one's been edited or the count no longer matches the old default catalog size.
...
- **Every Groq call has a hard timeout** (`src/ai/groqClient.js`, ~5s default, overridable per-call via a `timeoutMs` option — `GROQ_TIMEOUT_MS` only changes the default) — a single shared wrapper used by `classify.js`, `intentDetector.js`, `factualQA.js`, `orchestrator.js`, and `translate.js` so this is one place to get right, not several copies of `fetch()` boilerplate that can drift. Before this existed, a hung Groq response could block a customer's entire message indefinitely with no reply at all; now every call aborts on timeout and falls through to its own keyword/deterministic fallback. `workflowGenerator.js` (the admin dashboard's "generate a business from a description" feature) was found live to still be calling Groq directly, bypassing this entirely — fixed, with a longer 15s override rather than the 5s default since its `max_tokens: 2000` completion genuinely needs more time than the other, much shorter call sites. Verified live (real Groq calls, `GROQ_API_KEY` set): p50 ~1.1s, p95 ~1.8s end-to-end reply time. A forced-timeout integration test (`tests/latencyIntegration.test.js`) proves the full pipeline still replies within a bounded window even when Groq is completely unreachable, not hung forever.
...
## Account creation & activation, and where the newer unified plan stands

A later, more detailed product plan ("Complete Unified Implementation Plan") reorganized everything above into an access-control model, a sales-assisted account-creation flow, a two-hub dashboard IA, and a WhatsApp number strategy. Here's an honest accounting of its first four sections against what's actually in this codebase — audited, then implemented where it was a real gap, not assumed.

**1. Access Control & Tenancy Model — already implemented**, just under different names than the plan uses. `platform_admin` (Platform Admin), `admin` (the plan's "Provider/Business Owner" — this codebase's tenant-level owner role), and `provider` (the plan's "Team Member" — a single staff/room login) map exactly onto Section 8's existing role model. Every `/api/dashboard/*` route already enforces `tenantId` isolation at the store layer (Sections 5 and 8's own invariant #1 in `docs/ARCHITECTURE.md`); `/api/platform/*` is already `platform_admin`-only; a tenant-scoped account already has no route to create a second tenant. No code changes needed here.

**2. Marketing Website Account Creation & Onboarding Flow — the one real gap, now closed.** Self-serve signup used to create a tenant and grant instant dashboard access in the same request — deliberately, and documented as such at the time ("NOT a hard gate... not a signup approval bottleneck"). The unified plan calls for sales-assisted onboarding instead: verify the signer's email, create the account in a genuinely pending state, and require a platform admin to review and activate it before the dashboard is reachable. This is a real reversal of previously-shipped, tested behavior, done deliberately here, not silently:

- **`POST /api/signup/request-otp`** (`src/store/signupOtpStore.js`) generates a 6-digit code, hashed at rest with the same single-use/short-expiry discipline `passwordResetStore.js` already established, "sent" via the existing simulated `emailSender.js`. Rate-limited per email (`src/infra/rateLimit.js`'s `isOtpRateLimited`).
- **`POST /api/signup`** now requires a valid `otp` for that email before creating anything.
- **`requireAuth()`** now blocks a `"pending"` tenant's own users the same way it already blocked `"suspended"`/`"cancelled"` — a real reversal of the route's own previous comment, called out explicitly in both places it changed. A self-signed-up admin can log in (the account is real) but every `/api/dashboard/*` route 403s with `{ pendingActivation: true }` until a platform admin activates the tenant (`PATCH /api/platform/tenants/:id/status`, which already existed).
- **The React app** (`AuthContext.jsx`, `App.jsx`) shows a dedicated "Almost there" screen instead of a broken multi-panel dashboard full of repeated 403s, or a misleading bounce back to the login form.
- **The marketing signup page** (`public/marketing/signup.html`/`auth.js`) is now two-step (request code → verify code + create account), and its success state is "you're on the list," not an instant redirect into a dashboard that would just reject the session.
- **Live-verified end to end**, not just unit-tested: signed up through the real form, read the code back out of the (simulated) email log, verified it, confirmed the pending screen renders, activated the tenant via `PATCH .../status` as a real platform_admin session, and confirmed the *exact same* browser session — no new login — immediately unlocked the real dashboard. `tests/http/auth.test.js` covers the OTP lifecycle (missing/wrong/single-use/resend-invalidates-earlier), the pending gate, and the full activation path; every other HTTP test file's shared `signupAndActivate()` fixture (`tests/http/_setup.js`) went through the same update. Full suite (193 tests) and typecheck both pass.
- **Not built**: a real email/notification channel for the platform team (still simulated/logged, same honest gap `emailSender.js` already had) and a "magic link" first-login flow — activation instead relies on the password the customer already set at signup, reusing far more existing infrastructure for the same net effect.

**3. Provider Dashboard Structure (two-hub IA) — deliberately not attempted.** This section asks for a full redesign of the dashboard's information architecture (splitting every existing panel into a "Business Operations Hub" and a "Booking & Availability Hub"), plus a command palette, virtualized tables, optimistic updates with undo toasts, skeleton/empty/error states throughout, and a mobile bottom-nav layout. That's a materially larger undertaking than anything else in this section — closer in scope to the entire React rewrite (Section 13/Item 4) than to a single feature — and not something to attempt as a drive-by alongside an access-control and onboarding overhaul. Flagged here rather than silently skipped.

**4. WhatsApp Number Strategy — half already real, half needs a product decision this document can't make alone.** The "own-number add-on" half is already fully implemented: `tenants.whatsapp_phone_number_id` is exactly a dedicated per-tenant number, and the webhook already resolves an incoming message's tenant by matching it (`tenantStore.getByPhoneNumberId`, Section 8.3). The "universal number" half — one BookPilot-owned number shared by every tenant without their own, disambiguated some other way (a keyword, a deep-link parameter, a first-message business picker) — genuinely isn't implemented, and reasonably can't be without two things this pass doesn't have: a real Meta-owned shared number to test the disambiguation flow against, and a decision on *which* disambiguation mechanism to build (each has real, different tradeoffs — a deep link is invisible to a customer who finds the number some other way; a keyword prefix adds friction to every first message; a picker adds a conversational step for every universal-number customer, forever). Writing routing/tenant-resolution code that can't be verified end to end isn't something to ship quietly — flagged here as a real product decision, not a coding gap.

### Continuing past section 4: what's been added since

**Block 15 (Platform Admin Portal) — a minimal first slice, not the full portal.** The OTP/activation gate above had no UI at all once it shipped — activating a signup meant `curl`-ing `PATCH /api/platform/tenants/:id/status` by hand. `PlatformAdminView.jsx` gives a `platform_admin` session a real landing page: a tenant list sorted pending-first with plan/status/WhatsApp-connected/booking+user counts, Activate/Suspend/Reactivate/Cancel actions, and a Create Tenant modal. `App.jsx` now routes `platform_admin` sessions here instead of the regular dashboard (which 403s them — they have no `tenantId` to fetch dashboard data for). Health scores, impersonation, universal-number management, and system config are real, separate, larger work — not attempted here.

**Block 12 (SaaS Billing & Usage Metering Skeleton) — plans, live usage tracking, and a soft limit.** Matches the marketing site's own existing pricing tiers: Starter (free, 100 bookings/mo), Growth and Enterprise (unlimited). Deliberately a skeleton, not real recurring payment collection:

- **`src/engine/billing.js`** computes usage live from real booking rows every call — the same "computed on read, not a running counter" philosophy `analytics.js` already uses and documents, for the same reason (a counter that can drift from what it's summarizing is a worse problem than the scan it would save at this data size).
- **`GET /api/dashboard/billing`** — a tenant's own plan + this-month usage + whether they've hit their limit. Rendered as `BillingPanel.jsx` in the admin dashboard, with a usage bar and an upgrade-nudge banner once the limit is reached.
- **`PATCH /api/platform/tenants/:id/plan`** (+ `tenantStore.setPlan`) — the one lever that can change a tenant's plan at all; every creation path (self-signup, the platform admin's own Create Tenant modal) hardcodes `"free"` and nothing else ever touched it before this.
- **The limit is soft, deliberately** — `getUsageSummary()`'s `softLimitExceeded` flag is a dashboard signal, never an enforcement mechanism. The WhatsApp booking pipeline itself doesn't check it or reject anything; rejecting a real customer's booking because of the *operator's* plan choice is a materially different, far more consequential decision than showing a warning banner, and not one this pass makes unilaterally.
- **A real bug, found live while testing this**: `src/engine/billing.js` (like three other engine modules with the same shape — `analytics.js`, `calendarSync.js`, `paymentRefunds.js`) caches a `require()`'d store reference at module load time, the same pattern `workflowEngine.js` already needed a test-isolation fix for. None of the four were in `tests/http/_setup.js`'s cache-bust list, so any HTTP test file using more than one tenant across more than one `freshApp()` call was silently reading from the *first* test's database, not the current one — usage counts and analytics both firsthand affected, caught only because this pass's billing tests exercise exactly that pattern. Fixed by adding all four to the bust list.
- **Live-verified**: a fresh tenant's billing panel correctly shows 0/100 on the Starter plan; changed a real tenant's plan to Growth via the platform admin UI and confirmed the tenant's own billing panel immediately reflected "unlimited" on the next load. `tests/http/billing.test.js` covers this-month-only usage counting, the soft-limit flag, that the limit is never enforced as a hard block on the real booking pipeline, tenant isolation, and the plan-change route's own validation. Full suite (200 tests) and typecheck both pass.

**Block 13 (Notification / Reminder Engine) — 24h/2h pre-appointment reminders, reusing the existing delivery machinery end to end.** `src/infra/reminders.js` polls every 10 minutes (`scheduleReminders()`, `.unref()`'d, same pattern as `scheduleBackups()`/`startOutboundQueueWorker()`) and sends via `sendWithRetry()` — the same "a couple of immediate attempts, then the durable `outboundQueueStore`" delivery path the queue-position "you're next" alerts already use, not a second bespoke one just for this feature. Two independent thresholds, not one: a booking made 90 minutes before its slot was never eligible for the 24h reminder, but must still get the 2h one; a booking that already got its 24h reminder must still get the separate 2h one later. Each fires once (`bookings.reminder_24h_sent_at`/`reminder_2h_sent_at`, two nullable timestamps, never cleared) and is skipped entirely for a terminal (`cancelled`/`done`/`no_show`) or `serving` booking, or a suspended/cancelled tenant — the same rules `isTerminal()` (Item 6) and the webhook's own tenant-status gate already enforce elsewhere. A hotel stay only gets the 24h ("tomorrow") reminder, never the 2h one — its "appointment instant" is midnight of check-in day, and a "2 hours until your stay" notice for a multi-night booking would just be a wrong-shaped message. Post-booking CSAT already has its own, separate, working mechanism (`src/store/feedbackStore.js`, triggered on marking a booking complete — see "Post-booking commands" above) and wasn't migrated onto this engine; it didn't need to be. `tests/http/reminders.test.js` covers the pure window-boundary decision (`isDue()`, fast and deterministic, no real-clock dependency) plus one real end-to-end test through the actual send path proving a reminder fires with the correct message, marks itself sent, and never double-sends on a second run. Full suite (208 tests) and typecheck both pass; also live-verified the scheduler starts cleanly at boot with no errors.

**Block 14 (Multi-tenant Security Hardening) — session list/revoke, real this time; CSRF, API-key hashing, and webhook replay protection were already covered.** Auditing this block first: CSRF was already verified sufficient without a token scheme (`SameSite=Lax` + JSON-only body parsing, README's own Section 12 writeup), API keys are already SHA-256 hashed with a recognizable `bpk_` prefix (Section 14), and webhook replay protection already exists (`message.id` dedup, Section 0/`src/infra/dedupe.js`) — none needed rework. **Session list/revoke did**: sessions used to be purely stateless signed tokens (HMAC + expiry, no server-side record at all) — real, but with one gap: no way to end ONE session early. Every token now carries a random `sid` (`src/infra/auth.js`), recorded in a new `auth_sessions` table (`src/store/authSessionStore.js`) at login/signup and checked by `requireAuth()` on every request — the token itself stays the actual stateless bearer credential (no session-table lookup needed just to verify a signature and expiry), this is *only* an additional "has this one been revoked" check.

- **`GET /api/auth/sessions`** / **`DELETE /api/auth/sessions/:id`** — a user's own devices, and logging one out remotely (a shared computer they forgot to sign out of, a session they don't recognize) without resetting their password to invalidate everything at once. `SessionsPanel.jsx` in the admin dashboard.
- **`POST /api/auth/logout`** now revokes server-side too, not just clears the cookie client-side — a leaked/copied cookie value stops working immediately instead of remaining valid for the rest of its 12h natural lifetime.
- **Live-verified**: listed a real session, confirmed `isCurrent` correctly flagged, and confirmed the panel renders real device/sign-in/expiry info in the actual dashboard. `tests/http/sessions.test.js` covers two-"device" independence, that a revoked session's cookie is rejected on its *very next* request (not just no-longer-listed), that revoking is scoped to the caller's own user id (can't touch another user's session by guessing its id), and that logout revokes server-side. Full suite (213 tests) and typecheck both pass.
- **Not built**: MFA for platform admins. A real, separate feature (TOTP secret generation + QR enrollment + a second login step + backup codes, or an equivalent SMS-based flow) needing its own product decisions (mandatory or optional? which second factor?) this pass doesn't make unilaterally — flagged here rather than half-built. "Expanded isolation tests" is ongoing rather than a discrete deliverable — every item in this session added its own tenant-isolation regression coverage as it went (see `tests/http/workflows.test.js`, `billing.test.js`, `sessions.test.js` above, each with a dedicated cross-tenant test).

**Block 6 leftover — Vitest + React Testing Library smoke tests.** The React dashboard (`frontend/`) has been built out extensively across this session (the setup checklist, billing panel, sessions panel, platform admin view, the workflow editor + marketplace) with zero automated frontend coverage — every one of those was verified live in a real browser session, which proves the feature works but isn't a regression net for the *next* unrelated change. `frontend/vite.config.js` now has a `test` block (Vitest reads Vite's own config — no separate `vitest.config.js` needed) with three smoke-test files (`src/components/__tests__/*.test.jsx`) covering `BillingPanel`, `SetupChecklistPanel`, and `SessionsPanel`: does each render real (mocked) API data without throwing, show the right numbers/labels, and handle an API error without crashing. Deliberately smoke tests, not exhaustive coverage — the live-browser verification already done for each component this session covers the deeper interaction paths. **A real environment issue, found live**: Vitest's default `forks` test-isolation pool (spawns a real child process per worker) just hung/timed out in this project's sandboxed dev environment, which restricts process spawning — switched to `pool: "threads"` (`worker_threads`, no new OS process), which passes the identical tests in ~6 seconds. `.github/workflows/ci.yml`'s frontend job now runs `npm test` before `npm run build`. React Router and the panels/ui component-folder split from the original Block 6 description are still not done — neither fixes a bug or unlocks anything else in this session's backlog, so both stay flagged rather than bundled in as a drive-by alongside adding test tooling.

**Block 19 (OpenAPI & Documentation) — a real, importable spec for the Public API; internal routes and runbooks deliberately out of scope.** `public/openapi.yaml`, served unauthenticated at `GET /openapi.yaml` and linked directly from the dashboard's API Keys panel, describes both `/api/v1/*` routes in full — every parameter, response field, and error status this section of the README already documents in prose, now also machine-readable (import into Postman, Insomnia, or Swagger Editor). Deliberately a plain static file, not a hosted Swagger UI — that would need a CDN script this project's own CSP (Section 12) is built to block, and the spec file itself is the actual deliverable most integrators want (their own tooling renders it). Scoped to the Public API specifically, not all ~70 internal dashboard routes: those change with every dashboard feature and are never consumed by a third party, so a full spec for them would be a much larger, much lower-value undertaking than documenting the one surface external integrators actually touch. Internal runbooks (activation, number onboarding, incident response) aren't written either — this project doesn't have an on-call rotation or a support team yet for a runbook to serve. `tests/http/openapi.test.js` confirms the route serves the real file and describes the real API (both endpoint paths, the auth scheme) rather than a stale copy. Full suite (214 tests) and typecheck both pass; live-verified the route serves a real 200 with the actual spec content.

### Business listing: own real data, not a demo catalog

The auto-seeded `workflows/*.json` demo catalog (haircut/hotel/makeup/medical/automobile) was always meant as a "here's a working example, go customize it" starting point — in practice, for an operator setting up a real product, it was clutter to notice and delete before their own businesses would even fit. This pass removes it as a default and closes two real bugs found while building/verifying its replacement.

- **A brand new tenant now starts with zero businesses.** `tenantWorkflowStore.seedDefaultsForTenant()` is no longer called from `POST /api/signup`, `POST /api/platform/tenants`, or the old per-boot backfill loop in `server.js` — it still exists purely as a test fixture (`tests/http/_setup.js` calls it directly so booking-flow tests keep a working catalog to exercise, decoupled from what production actually does). The public demo tenant (`bookpilot-live-demo`) had its seeded catalog and bookings wiped the same way, one-time, via a throwaway script — it's a real, empty tenant now, same as any other.
- **The "Add Business" form (`WorkflowEditorModal.jsx`) needs no JSON editing for the common case.** Business ID and booking-prefix are auto-derived from the name typed in (slugified, de-duplicated against existing ids — the same pattern `POST /api/signup` already uses for a tenant's own slug); the raw JSON/technical fields collapse under an "Advanced settings" toggle, open by default only when editing an existing business. Per provider: a photo can be uploaded from device (`POST /api/dashboard/upload-image`, multer, 5MB cap, JPEG/PNG/WEBP/GIF only, served back at `/uploads/<tenantId>/<file>`) or pasted as a URL — same field either way; a location can be set by clicking a pin on a free OpenStreetMap/Leaflet map, or by typing raw coordinates or pasting a Google Maps link (parsed for embedded `@lat,lng` / `q=lat,lng` coordinates, falling back to the raw text as a search query if none are found — Maps' own search-link format already accepts either).
- **Google Maps / share.google link auto-fill — name only, and that's a deliberate, verified limit, not a shortcut.** `POST /api/dashboard/resolve-maps-link` (`src/infra/mapsLinkResolver.js`) follows a pasted link's redirects and reads the business name straight off the resolved URL's own `q=`/`/place/` segment — no HTML scraping needed for that part, so it doesn't depend on Google's page markup staying stable. Verified live against real `share.google` links: they resolve to a Google **Search** results page, not a Maps place page, and that page is entirely client-rendered — no photo, address, or coordinates exist anywhere in what a plain server-side fetch receives. Getting those reliably would need the paid Google Places API (a real API key + billing setup) or a headless-browser render (heavy, fragile, and arguably against Google's terms for automated use) — neither was built unilaterally; photo and location stay on the manual upload/pin/coordinate fields above. Restricted to a small hostname allowlist (`share.google`, `google.com` and its subdomains, `goo.gl`) since it's a server-side fetch of a user-supplied URL — otherwise a straightforward SSRF vector.
- **Two real bugs found and fixed while reproducing a live report of this flow breaking**: (1) a business created through the old bare 2-step default template (pick a provider, pick a date, nothing after) auto-completed the booking the instant the last step was answered — no name collected, no review step, and no `confirmationTemplate` to send, so the customer got total silence right where they expected a confirmation, then a confusing "you already have a booking" reply on their next message (the booking WAS real, they just never saw it exist). Fixed two ways: `workflowEngine.js`'s `advanceOrFinish()` now builds a sensible confirmation message from whatever the session actually collected whenever a workflow has no `confirmationTemplate` of its own, and the "Add Business" default template is now a genuinely complete flow (provider → date → time → name → review & confirm) so a business built through the form can't end up in this state at all. (2) `src/ai/classify.js`'s keyword-fallback classifier did `workflow.keywords.some(...)` with no guard — every business made through the current admin form has no `keywords` field at all (it's not even exposed), so the bot **crashed outright** the moment a tenant had 2+ such businesses and a customer hit a reclassifiable step. A near-identical guard already existed in `loadWorkflows.js`, but only for the one-time static catalog load, never for a business created or edited afterward through the dashboard — the real, robust fix defaults to `[]` at the actual read sites instead.
- **Live-verified end to end**: reproduced the exact silent-booking bug via a real simulated WhatsApp conversation against a business shaped like the reported one, confirmed the fix produces a real confirmation send attempt (rejected only by this dev environment's WhatsApp sandbox allow-list, not an empty body); walked a full provider → date → time → name → confirm conversation through a freshly-created business using the new default template end to end, producing a real `status: "booked"` row; fetched three of a real batch of `share.google` links (one per business category) through the live endpoint and confirmed each correctly filled in a business name; confirmed a non-Google URL is rejected by the resolver's allowlist. Full backend suite (214 tests), frontend build and smoke tests, typecheck, and `npm audit --audit-level=high` all pass clean.

### Codebase reorganization — `server.js` split into per-domain route files
